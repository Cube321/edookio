...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const colors = [
        'rgba(232,15,136,1)', 'rgba(0,123,255,1)', 'rgba(40,167,69,1)',
    ...
    const datasets5 = facultyNames.map((faculty, i) => ({
        label: faculty,
    ...
    const allDataPoints5 = datasets5.flatMap(dataset => dataset.data);
    const maxValue5 = Math.max(...allDataPoints5);

    // Optional: Add some padding to the maximum value for better visualization
    const maxYValue5 = maxValue5 + (0.1 * maxValue5);
